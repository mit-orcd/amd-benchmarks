...
Two results here, and the second one nearly became a false finding.

**EP loads and runs on the MAD-pinned image.** That alone corrects the record: `kimi-k3-base.md`
and earlier notes state that MXFP4 experts and EP are mutually exclusive, based on
`rocm/atom-dev:latest` raising `NotImplementedError: a16w4 (bf16 A x MXFP4 W) SiTUv2 is not
supported: expert-parallel masking`. That failure is **kernel-path-specific, not architectural**.
MAD sets `ATOM_USE_TRITON_MOE=0` / `AITER_USE_GROUPED_GEMM=0`, selecting a different MoE kernel
that has no such restriction. A freshly-pulled `:latest` (2026-08-20, digest confirmed changed)
still fails the same way — so the constraint is the SiTUv2 kernel, and MAD's kernel set is the
way around it.

- Baseline (TP-only): `kimi_mad_20260818_223148`, `--max-num-seqs 64`
- EP enabled: `kimi_ep_mad_20260820_042519`, `--max-num-seqs 256`

| Concurrency | tok/s TP-only | tok/s EP | EP/TP | TPOT TP-only (ms) | TPOT EP (ms) | TTFT TP-only (ms) | TTFT EP (ms) | Comparable? |
|---:|---:|---:|---:|---:|---:|---:|---:|:--|
| 64 | 1,142.7 | 977.5 | **0.86x** | 52.97 | 56.72 | 281.1 | 287.7 | yes — both under cap |
| 128 | 1,187.3 | 1,633.7 | 1.38x | 53.35 | 77.50 | 49,456.2 | 392.0 | **no — confounded** |

**Only the c=64 row is a valid A/B.** The two runs did not use the same `--max-num-seqs` (64 vs
256), so at c=128 the baseline was admission-capped at 64 and queueing — its 49.5 s TTFT is the
signature, against 392 ms for EP. That 1.38x measures **the `max-num-seqs` cap, which section 4
already established as worth 2.1x**, not expert parallelism. Reading it as an EP win would have
re-attributed a known effect to a new cause.

**On the one clean point, EP costs 14%** (977.5 vs 1,142.7 tok/s at c=64), with TPOT up
52.97 -> 56.72 ms. **The bandwidth hypothesis is not supported.** The reasoning that motivated it —
HBM ~29% utilized, XGMI ~1%, so move traffic to the idle resource — treats the two as
interchangeable, and they are not. All-to-all sits on the critical path *twice* per MoE layer
(dispatch + combine), each with a synchronization barrier; across 92 MoE layers that is 184
serialized round-trips per token. At c=64 the added latency exceeds the HBM traffic saved, even
with ~99% of interconnect bandwidth spare.

**This is a single clean data point and should not be over-read.** EP's advantage grows with
batch size — more distinct experts activate per step, so the per-GPU weight-read saving rises
while all-to-all latency stays roughly fixed. c=64 is the regime where EP is *least* likely to
pay off. A matched-`max-num-seqs` sweep at c>=256 is the test that would settle it, and it was
not run.

**Method note.** This block is generated by `analyze_kimi_ep.py`, which picks its headline from
the highest-concurrency row. That heuristic selected the confounded c=128 point and reported
"EP helps — hypothesis supported, 38%". The comparability column and this text are the
correction; the script's automatic verdict should not be trusted without checking that both runs
share the same admission cap.
