...
NAME="atom-kimi-maxseqs"

TS=$(date +%Y%m%d_%H%M%S)
OUT=$LOG_ROOT/atom/kimi_maxseqs_$TS; mkdir -p "$OUT"
STATE=$OUT/STATE.txt
...
if [[ $fail -ne 0 ]]; then
  ...
  echo "MAXSEQS_STATUS=failed" >>"$STATE"; exit 1
fi
echo "MAXSEQS_STATUS=ok" >>"$STATE"
echo "MAXSEQS_SWEEP=$OUT" >>"$STATE"

say "generating results/kimi-k3-maxseqs.md"
$PY analyze_kimi_maxseqs.py "$OUT" -o "$BENCH_ROOT/results" >"$OUT/analyze.log" 2>&1
say "analyze rc=$? -> $BENCH_ROOT/results/kimi-k3-maxseqs.md"
