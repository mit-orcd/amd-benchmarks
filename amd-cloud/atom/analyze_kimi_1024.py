#!/usr/bin/env python3
"""Generate results/kimi-k3-maxseqs1024.md from the raised --max-num-seqs experiment.
...
NEW run. Old numbers are never shown. Writes only kimi-k3-maxseqs1024.md (+ .csv); it does
...
def main():
    ...
    # CSV
    import csv as _csv
    with (a.out / "kimi-k3-maxseqs1024.csv").open("w", newline="") as fh:
    ...
    A("8 × MI355X node via ATOM, TP=8, with **`--max-num-seqs` raised to 1024** to")
    ...
    A("| This table as CSV | `results/kimi-k3-maxseqs1024.csv` |")
    ...
    (a.out / "kimi-k3-maxseqs1024.md").write_text("\n".join(Lx) + "\n")
    print(f"wrote {a.out}/kimi-k3-maxseqs1024.md and .csv ({len(rows)} concurrency points)")
