...
This repo tracks text only (scripts, plans, markdown, CSV, logs) and enforces it with a
`.gitignore` + pre-commit size hook. See **[`REPO_TRACKING.md`](REPO_TRACKING.md)** for what's
tracked, where bulk artifacts (model weights, container images) live instead, and the
one-time hook setup for a fresh clone.
