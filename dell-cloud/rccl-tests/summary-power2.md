...
## Q: Does RCCL suggest users only run jobs with a power-of-2 number of GPUs on this server?

**RCCL has no explicit written documentation stating "use power-of-2 GPU counts."** However, inspecting the installed ROCm 7.2.3 on this server (`/opt/rocm-7.2.3/share/rccl/msccl-algorithms/`) makes the implicit stance clear: every bundled MSCCL plan targets only power-of-2 GPU counts.

| Plan family | GPU count (`ngpus=`) |
|-------------|----------------------|
| allreduce-allpairs-\* | 8 |
| allgather-direct-\* | 16, 32 |
| alltoall-\* | 8 |

No MSCCL plan exists for N = 5, 6, 7, or any other non-power-of-2 count. When RCCL runs at those sizes it falls back to its generic ring algorithm — exactly the path that produces the ~38 GB/s busbw cliff (§1–2) versus ~381 GB/s at N=8.

**Practical answer for this MI355X server with ROCm 7.2.3: effectively yes**, not because of an official mandate, but because AMD ships tuned MSCCL plans only for {8, 16, 32} GPUs. Non-power-of-2 counts fall through to the generic ring fallback with a ~50% bandwidth penalty (§2), and no env-var workaround recovers it (§2).

**The "power-of-2 only" framing is slightly too broad, however.** The measured data shows N=3 hits ~75 GB/s (same ballpark as N=2 at 61 GB/s), because a 3-ring is trivially balanced on any topology. The cliff is specific to N ∈ {5, 6, 7}. The more accurate guidance for this server is:

> **Avoid N ∈ {5, 6, 7}.** N ∈ {2, 3, 4, 8} all produce healthy collective bandwidth. "Power-of-2" is sufficient but not necessary — N=3 is safe too.
