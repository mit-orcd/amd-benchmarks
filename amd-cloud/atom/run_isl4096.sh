...
PORT="${PORT:-8013}"
MAXSEQS="${MAXSEQS:-256}"
CONC="${MAXSEQS_CONC:-64 128 256}"
ISL="${ISL:-4096}"; OSL="${OSL:-1024}"
NAME="atom-kimi-isl4096"

TS=$(date +%Y%m%d_%H%M%S)
OUT=$LOG_ROOT/atom/kimi_isl4096_$TS; mkdir -p "$OUT"
STATE=$OUT/STATE.txt
...
if [[ $fail -ne 0 ]]; then
  ...
  echo "ISL4096_STATUS=failed" >>"$STATE"; exit 1
fi
echo "ISL4096_STATUS=ok" >>"$STATE"
echo "ISL4096_SWEEP=$OUT" >>"$STATE"

say "generating results/kimi-k3-isl4096.md"
$PY analyze_isl4096.py "$OUT" -o "$BENCH_ROOT/results" >"$OUT/analyze.log" 2>&1
say "analyze rc=$? -> $BENCH_ROOT/results/kimi-k3-isl4096.md"
say "MAX-NUM-SEQS EXPERIMENT DONE"
