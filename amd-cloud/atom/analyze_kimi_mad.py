#!/usr/bin/env python3
"""Generate results/kimi-k3-mad.md from the MAD-recipe rerun.
...
NEW run. Old numbers are never shown. Writes only kimi-k3-mad.md (+ .csv); it does
...
def main():
    ...
    # CSV
    import csv as _csv
    with (a.out / "kimi-k3-mad.csv").open("w", newline="") as fh:
    ...
    A("# Kimi-K3 on 8 × MI355X — MAD-recipe rerun (compute and communication analysis)")
    A("")
    A("Serving `moonshotai/Kimi-K3` (2.78 T params, 1.5 TB MXFP4 checkpoint) on a single")
    A("8 × MI355X node via ATOM, TP=8, using **AMD's official MAD benchmark recipe**")
    A("([ROCm/MAD](https://github.com/ROCm/MAD/blob/develop/benchmark/kimi_k3/README.md)).")
    A(f"Source run: `{a.sweep.name}`.")
    A("")
    A("> This file reports **only** the MAD-recipe run. The earlier run using")
    A("> `rocm/atom-dev:latest` with the ATOM in-repo recipe is in `kimi-k3-base.md`; the two are")
    A("> kept separate on purpose so neither overwrites the other. Rationale and the full")
    A("> config diff are in `notes-kimi-k3.md`.")
    A("")
    A("**Run configuration** (from the server log and launch command, not assumed):")
    A("")
    A("| Setting | Value |")
    A("|---|---|")
    A("| Image | `rocm/atom-dev:rocm7.2.4_...20260727_kimi_k3` (MAD-pinned, Kimi-K3-specific) |")
    A("| Parallelism | `tensor_parallel_size=8`, PP=1, DP=1, EP off |")
    A("| Quantization | MXFP4 routed experts; kernel set selected by MAD env vars (no `--online_quant_config`) |")
    A("| KV cache dtype | fp8 |")
    A("| `max_model_len` / `max_num_seqs` | 16384 / 64 |")
    A("| `max_num_batched_tokens` | 10240 |")
    ...
    A("| MAD env vars | `ATOM_USE_TRITON_GEMM=1`, `ATOM_USE_TRITON_MOE=0`, `AITER_USE_GROUPED_GEMM=0`, `AITER_FLYDSL_FORCE=1`, `ATOM_USE_UNIFIED_ATTN=1`, `ATOM_FORCE_ATTN_TRITON=1`, +loader/timeout vars |")
    A("")
    ...
    A("")
    A(f"MAD's sweep deliberately runs past `--max-num-seqs 64`, so concurrency above 64")
    A(f"queues rather than being rejected — the TTFT p99 column is where that shows up.")
    A("")
    A("### Achieved TFLOP/s")
    A("")
    ...
    A("")
    A("**2. `max_num_seqs=64` remains the binding limit, not hardware** — this sweep drives")
    A("past it, so concurrency above 64 queues rather than increasing in-flight batch.")
    A("")
    ...
    A("| This table as CSV | `results/kimi-k3-mad.csv` |")
    ...
    (a.out / "kimi-k3-mad.md").write_text("\n".join(Lx) + "\n")
    print(f"wrote {a.out}/kimi-k3-mad.md and .csv ({len(rows)} concurrency points)")
