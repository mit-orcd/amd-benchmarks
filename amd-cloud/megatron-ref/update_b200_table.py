...
def main():
    ...
    if delta >= 1.03:
        L += [f"AMD Cloud comes out **{(delta-1)*100:.0f}% ahead of Dell Cloud** on identical "
              f"workload and image family. The most likely cause is that this host does "
              f"**not** set `HSA_OVERRIDE_GFX_VERSION=9.4.2` — Dell had to, because their "
              f"image's PyTorch lacked gfx950 code objects, so their run executed gfx942 "
              f"kernels on gfx950 silicon. Newer ROCm/driver on this host is a secondary "
              f"candidate. This also moves the MI355X-vs-B200 ratio from "
