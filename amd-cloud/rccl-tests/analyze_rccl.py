...
def main():
    ...
    # Figure, generated by plot_rccl_busbw.py from the same rccl.csv this script writes.
    # Embedded here rather than only listed under Source data -- the cliff is far easier to
    # see than to read out of the matrix above.
    L += ["![RCCL busbw vs GPU count — the non-power-of-2 cliff](rccl_busbw.png)", "",
          "Blue = ring-based collectives (need a complete ring across the mesh); orange = "
          "pairwise-sendrecv collectives (route directly to/from a root, no ring required). "
          "The shaded band is N=5,6,7. Ring-based collectives collapse ~3.5-5x inside it and "
          "snap back at N=8; `sendrecv` is flat throughout because a single point-to-point "
          "exchange never depends on ring construction. Regenerate with "
          "`plot_rccl_busbw.py results/rccl.csv results/rccl_busbw.png`.", ""]
